...
export const solicitarCambio = async (req, res, next) => {
  ...
  try {
    ...
    const { rows: [prov] } = await client.query(
      `SELECT datos_bancarios_estado FROM tesoreria_proveedores WHERE id = $1 FOR UPDATE`,
      [id]
    );
    if (!prov) { await client.query('ROLLBACK'); return res.status(404).json({ error: 'Proveedor no encontrado' }); }
...
export const verificar = async (req, res, next) => {
  ...
  try {
    ...
    const { rows: [sol] } = await client.query(
      `SELECT * FROM tesoreria_proveedores_datos_bancarios WHERE id = $1 FOR UPDATE`,
      [id]
    );
    if (!sol) { await client.query('ROLLBACK'); return res.status(404).json({ error: 'Solicitud no encontrada' }); }
